...
#endif
#ifdef __APPLE__
#define RNWebGLSysLog(fmt, ...) iOSLog("RNWebGL: " fmt, ##__VA_ARGS__)
#endif
#else
...
private:
  std::unordered_map<RNWebGLObjectId, GLuint> objects;
  static std::atomic_uint nextObjectId;
  
public:
  inline RNWebGLObjectId createObject(void) noexcept {
    return nextObjectId++;
  }
...
#define GL_DEPTH_STENCIL_ATTACHMENT 0x821A
  ...
  static inline std::shared_ptr<const char*> jsValueToSharedStringArray(JSContextRef ctx, JSValueRef jsVal, int *length) {
    ...
    *length = jsArrayGetCount(ctx, jsObj);
    ...
    for (int i = 0; i < *length; i++) {
      ...
      strings[i] = JSValueToUTF8CStringMalloc(ctx, item, nullptr);
    }
  ...
  inline void prepareTypedArrayAPI(JSContextRef jsCtx) {
    ...
    if (iOSGetOperatingSystemVersion().majorVersion >= 10) {
      return;
    }
#endif
...
static JSValueRef RNWebGLNativeStatic_##name(JSContextRef jsCtx,         \
...
inline JSValueRef RNWebGLNativeInstance_##name(JSContextRef jsCtx,       \
JSObjectRef jsFunction,   \
JSObjectRef jsThis,       \
size_t jsArgc,            \
const JSValueRef jsArgv[], \
...
_WRAP_METHOD_INTERNAL(name, JS_ARGC(__VA_ARGS__), isWebGL2Method) { \
...
#undef _GET_PARAMETER_UNIMPL
  ...
  _WRAP_METHOD(bindBuffer, 2) {
    JS_UNPACK_ARGV(GLenum target, RNWebGLObjectId fBuffer);
  ...
  _WRAP_METHOD(deleteBuffer, 1) {
    JS_UNPACK_ARGV(RNWebGLObjectId fBuffer);
    addToNextBatch([=] {
...
JS_UNPACK_ARGV(RNWebGLObjectId f);            \
GLboolean glResult;                           \
...
#define _WRAP_WEBGL2_METHOD_IS_OBJECT(type) _WRAP_METHOD_IS_OBJECT_INTERNAL(type, true)
  ...
  _WRAP_METHOD(bindFramebuffer, 2) {
    ...
    if (JSValueIsNull(jsCtx, jsArgv[1])) {
      ...
      RNWebGLObjectId fFramebuffer = JSValueToNumberFast(jsCtx, jsArgv[1]);
  ...
  _WRAP_METHOD(deleteFramebuffer, 1) {
    JS_UNPACK_ARGV(RNWebGLObjectId fFramebuffer);
    addToNextBatch([=] {
  ...
  _WRAP_METHOD(framebufferRenderbuffer, 4) {
    JS_UNPACK_ARGV(GLenum target, GLenum attachment, GLenum renderbuffertarget, RNWebGLObjectId fRenderbuffer);
    addToNextBatch([=] {
  ...
  _WRAP_METHOD(framebufferTexture2D, 5) {
    JS_UNPACK_ARGV(GLenum target, GLenum attachment, GLenum textarget, RNWebGLObjectId fTexture, GLint level);
    addToNextBatch([=] {
  ...
  _WRAP_WEBGL2_METHOD(framebufferTextureLayer, 5) {
    JS_UNPACK_ARGV(GLenum target, GLenum attachment, RNWebGLObjectId texture, GLint level, GLint layer);
    addToNextBatch([=] {
  ...
  _WRAP_WEBGL2_METHOD(invalidateSubFramebuffer, 6) {
    JS_UNPACK_ARGV(GLenum target);
    JS_UNPACK_ARGV_OFFSET(2, GLint x, GLint y, GLint width, GLint height);
    size_t length;
    auto attachments = jsValueToSharedArray(jsCtx, jsArgv[1], &length);
    addToNextBatch([=] {
  ...
  _WRAP_METHOD(deleteRenderbuffer, 1) {
    JS_UNPACK_ARGV(RNWebGLObjectId fRenderbuffer);
    addToNextBatch([=] {
      GLuint renderbuffer = lookupObject(fRenderbuffer);
  ...
  _WRAP_METHOD(bindTexture, 2) {
    ...
    if (JSValueIsNull(jsCtx, jsArgv[1])) {
      ...
      RNWebGLObjectId fTexture = JSValueToNumberFast(jsCtx, jsArgv[1]);
  ...
  _WRAP_METHOD(deleteTexture, 1) {
    JS_UNPACK_ARGV(RNWebGLObjectId fTexture);
    addToNextBatch([=] {
  ...
  _WRAP_WEBGL2_METHOD(texImage3D, 10) {
    ...
    JS_UNPACK_ARGV(target, level, internalformat, width, height, depth, border, format, type);
    ...
    // Nothing worked...
    throw std::runtime_error("RNWebGL: Invalid pixel data argument for gl.texImage3D()!");
  }
  ...
  _WRAP_WEBGL2_METHOD(texSubImage3D, 11) {
    ...
    JS_UNPACK_ARGV(target, level, xoffset, yoffset, zoffset, width, height, depth, format, type);
    ...
    // Nothing worked...
    throw std::runtime_error("RNWebGL: Invalid pixel data argument for gl.texSubImage3D()!");
  }
  ...
  _WRAP_METHOD(attachShader, 2) {
    JS_UNPACK_ARGV(RNWebGLObjectId fProgram, RNWebGLObjectId fShader);
    addToNextBatch([=] { glAttachShader(lookupObject(fProgram), lookupObject(fShader)); });
    return nullptr;
  }
  
  _WRAP_METHOD(bindAttribLocation, 3) {
    JS_UNPACK_ARGV(RNWebGLObjectId fProgram, GLuint index);
  ...
  _WRAP_METHOD(compileShader, 1) {
    JS_UNPACK_ARGV(RNWebGLObjectId fShader);
  ...
  _WRAP_METHOD(deleteProgram, 1) {
    JS_UNPACK_ARGV(RNWebGLObjectId fProgram);
  ...
  _WRAP_METHOD(getProgramParameter, 2) {
    JS_UNPACK_ARGV(RNWebGLObjectId fProgram, GLenum pname);
    GLint glResult;
  ...
  _WRAP_METHOD(getShaderParameter, 2) {
    JS_UNPACK_ARGV(RNWebGLObjectId fShader, GLenum pname);
    GLint glResult;
  ...
  inline JSValueRef getShaderOrProgramStr(JSContextRef jsCtx, const JSValueRef jsArgv[],
    ...
    JS_UNPACK_ARGV(RNWebGLObjectId fObj);
    GLint length;
    std::string str;
    addBlockingToNextBatch([&] {
  ...
  _WRAP_METHOD(getFragDataLocation, 2) {
    JS_UNPACK_ARGV(RNWebGLObjectId program);
...
return nullptr;                                                     \
    ...
    _WRAP_METHOD(vertexAttribIPointer, 5) {
      JS_UNPACK_ARGV(GLuint index, GLuint size, GLenum type, GLsizei stride, GLint offset);
...
#undef _WRAP_METHOD_VERTEX_ATTRIB_V
    ...
    _WRAP_WEBGL2_METHOD(drawElementsInstanced, 5) {
      JS_UNPACK_ARGV(GLenum mode, GLsizei count, GLenum type, GLint offset, GLsizei instanceCount);
      addToNextBatch([=] {
    ...
    _WRAP_WEBGL2_METHOD(drawRangeElements, 6) {
      JS_UNPACK_ARGV(GLenum mode, GLuint start, GLuint end, GLsizei count, GLenum type, GLint offset);
      addToNextBatch([=] {
...
JS_UNPACK_ARGV(GLenum buffer, GLint drawbuffer);                  \
...
#undef _WRAP_METHOD_CLEAR_BUFFER
    ...
    _WRAP_WEBGL2_METHOD(deleteQuery, 1) {
      JS_UNPACK_ARGV(RNWebGLObjectId fQuery);
      addToNextBatch([=] {
    ...
    _WRAP_WEBGL2_METHOD(beginQuery, 2) {
      JS_UNPACK_ARGV(GLenum target, RNWebGLObjectId query);
    ...
    _WRAP_WEBGL2_METHOD(getQuery, 2) {
      ...
      _WRAP_WEBGL2_METHOD(getQueryParameter, 2) {
        JS_UNPACK_ARGV(RNWebGLObjectId query, GLenum pname);
        GLuint params;
      ...
      _WRAP_WEBGL2_METHOD(deleteSampler, 1) {
        JS_UNPACK_ARGV(RNWebGLObjectId fSampler);
        addToNextBatch([=] {
      ...
      _WRAP_WEBGL2_METHOD(bindSampler, 2) {
        JS_UNPACK_ARGV(GLuint unit, RNWebGLObjectId sampler);
      ...
      _WRAP_WEBGL2_METHOD(deleteTransformFeedback, 1) {
        JS_UNPACK_ARGV(RNWebGLObjectId fTransformFeedback);
        addToNextBatch([=] {
      ...
      _WRAP_WEBGL2_METHOD(bindTransformFeedback, 1) {
        JS_UNPACK_ARGV(GLenum target, RNWebGLObjectId transformFeedback);
      ...
      _WRAP_WEBGL2_METHOD(transformFeedbackVaryings, 3) {
        JS_UNPACK_ARGV(RNWebGLObjectId program);
        JS_UNPACK_ARGV_OFFSET(2, GLenum bufferMode);
        int length;
      ...
      _WRAP_WEBGL2_METHOD(bindBufferBase, 3) {
        JS_UNPACK_ARGV(GLenum target, GLuint index, RNWebGLObjectId buffer);
        addToNextBatch([=] { glBindBufferBase(target, index, lookupObject(buffer)); });
        return nullptr;
      }
      
      _WRAP_WEBGL2_METHOD(bindBufferRange, 5) {
        JS_UNPACK_ARGV(GLenum target, GLuint index, RNWebGLObjectId buffer, GLint offset, GLsizei size);
      ...
      _WRAP_WEBGL2_METHOD(getActiveUniforms, 3) {
        JS_UNPACK_ARGV(RNWebGLObjectId program);
        JS_UNPACK_ARGV_OFFSET(2, GLenum pname);
        size_t length;
      ...
      _WRAP_WEBGL2_METHOD(getActiveUniformBlockName, 2) {
        JS_UNPACK_ARGV(RNWebGLObjectId fProgram, GLuint uniformBlockIndex);
        ...
        return JSValueMakeStringFromUTF8CString(jsCtx, blockName.c_str());
      }
      
      _WRAP_WEBGL2_METHOD(uniformBlockBinding, 3) {
        JS_UNPACK_ARGV(RNWebGLObjectId program, GLuint uniformBlockIndex, GLuint uniformBlockBinding);
        addToNextBatch([=] {
      ...
      _WRAP_WEBGL2_METHOD(deleteVertexArray, 1) {
        JS_UNPACK_ARGV(RNWebGLObjectId fVertexArray);
        addToNextBatch([=] {
      ...
      _WRAP_WEBGL2_METHOD(bindVertexArray, 1) {
        JS_UNPACK_ARGV(RNWebGLObjectId vertexArray);
...
&RNWebGLContext::RNWebGLNativeStatic_##name)
