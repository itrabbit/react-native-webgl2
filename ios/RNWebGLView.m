...
- (void)maybeCallSurfaceCreated {
    ...
    if (_onSurfaceCreate && _glContext.isInitialized && _isLayouted) {
        RNWebGLContextId ctxId = _glContext.contextId;
        RNWebGLContextSetDefaultFramebuffer(ctxId, _msaaFramebuffer);
        _onSurfaceCreate(@{ @"ctxId": @(ctxId) });
...
- (void)resizeViewBuffersToWidth:(short)width height:(short)height {
    ...
    [_glContext runAsync:^{
        ...
        if (self->_glContext.isInitialized) {
            RNWebGLContextSetDefaultFramebuffer(self->_glContext.contextId, self->_msaaFramebuffer);
        }
...
- (void)draw {
    // ctxId may be unset if we get here (on the UI thread) before RNWebGLContextCreate(...) is
    // called on the JS thread to create the GL context and save its id (see RNWebGLContext.initializeContextWithBridge method).
